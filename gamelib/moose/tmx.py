...
class Tileset(object):
    ...
    @classmethod
    def fromxml(cls, tag, firstgid=None, base_dir=None):
    ...
    @staticmethod
    def _fix_relative_filenames(filename, base_dir):
...
class ObjectLayer(object):
    ...
    def collide(self, rect, propname):
        ...
        for object in self.get_in_region(rect.left, rect.top, rect.right,
                rect.bottom):
            if propname in object or propname in self.properties or propname == object.name:
                r.append(object)
        return r
    ...
    def get_in_region(self, x1, y1, x2, y2):
        ...
        '''
        retval = [obj for obj in self.objects if obj.intersects(x1, y1, x2, y2)]
        # print "{} objects intersecting <{},{}|{},{}>".format(len(retval), x1, y1, x2, y2)
        return retval
...
class TileMap(object):
    ...
    def load(cls, filename, viewport):
        ...
        for tag in map.findall('tileset'):
            base = os.path.abspath(os.path.dirname(filename))
            tilemap.tilesets.add(Tileset.fromxml(tag, base_dir=base))
